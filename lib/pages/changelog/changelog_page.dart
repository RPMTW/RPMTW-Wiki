...
class _ChangelogPageState extends State<ChangelogPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: BasePage(builder: (context) {
        return FutureBuilder<List<WikiChangelog>>(
          future: load(),
          builder: (context, snapshot) {
            if (snapshot.hasData) {
              List<WikiChangelog> changelogs = snapshot.data!;
              return SizedBox(
                height: Utility.getScreenSize(context).height * 0.8,
                width: Utility.getScreenSize(context).width,
                child: Column(
                  children: [
